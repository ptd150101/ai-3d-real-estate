...
app = FastAPI(
    ...
    version="2.0.0",
    description="Multi-agency AI and immersive real-estate marketplace with payments, contracts, valuation, recommendations, reconstruction, AR/VR and mobile.",
    lifespan=lifespan,
)
app.add_middleware(
    CORSMiddleware,
...
async def request_context(request: Request, call_next):
    ...
    # StaticFiles serves public assets only. Private objects must always pass
    # through authenticated download endpoints, even when local storage is used.
    storage_parts = PurePosixPath(request.url.path).parts if request.url.path.startswith("/storage") else ()
...
@app.get("/")
def root():
    return {"name": settings.app_name, "version": "2.0.0", "docs": "/docs", "health": "/health"}
