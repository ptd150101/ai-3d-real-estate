...
def create_access_token(subject: str, role: str, expires_minutes: int | None = None) -> str:
    ...
    now = int(time.time())
    payload: dict[str, Any] = {"sub": subject, "role": role, "iat": now, "exp": now + 60 * (expires_minutes or settings.access_token_minutes), "jti": secrets.token_urlsafe(12)}
...
def decode_access_token(token: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not secrets.compare_digest(_b64url(expected), signature_b64): raise ValueError("invalid signature")
        payload = json.loads(_b64decode(payload_b64))
        if int(payload.get("exp", 0)) < int(time.time()): raise ValueError("token expired")
        return payload
